...
class RTMPClient extends RTMPProtocol
	...
	messageReceived:(msg)->
		if (msg.type is RTMPMessage.RPC or msg.type is RTMPMessage.RPC3) and msg.streamId == 0
			...
			@ev.emit('throttled')
			process.exit(3)
			@writeWinSize0 = @writeWinSize
			@writeWinSize = msg.data.readUInt32BE(0)# & 0xff
		else if msg.type is RTMPMessage.WIN_ACK_SIZE
			# logger.info('rtmp: Got ack-size', msg)
			@readWinSize = msg.data.readUInt32BE(0)
			@readWinSize0 = @bytesRead
		else
			# logger.info('rtmp: UNKNOWN MSG?', msg)
			# logger.info(hexy.hexy(msg.data))
			# data = msg.data
			# decoder = new AMF0Decoder(data)
			# while decoder.offset < decoder.buffer.length
			# 	logger.info('rtmp: 665', decoder.readValue())
...
class RTMPCommand
	...
	@fromMessage:(message)->
		...
		if message.type is RTMPMessage.RPC or message.type is RTMPMessage.RPC3
			...
			cmd.id = 0
		if typeof(cmd.id)=='undefined' or typeof(cmd.data)=='undefined'
			process.exit(3)
		cmd.args = []
			...
			if decoded_value.hasOwnProperty('object')
				if decoded_value.object.hasOwnProperty('rootCause')
					...
					if tmp.errorCode=='LOGIN-0007'
						# logger.info('rtmp: LOGIN-0007', decoded_value)
						process.exit(4)
					else if tmp.errorCode=='LOGIN-0001'
						# logger.warn("rtmp: new client version #{tmp.substitutionArguments[1]}")
						process.send({'event':'new_version', 'data':tmp.substitutionArguments[1]})
					else if tmp.errorCode=='PG-0014'
						# logger.info('rtmp: Not in a game') # No game
						cmd.args.push({error:'OB-1'})
						...
						if tmp.message=='Game is not observable '
							# logger.info('rtmp: Game not observable')
							cmd.args.push({error:'OB-2'})
						else if tmp.message=='Game has not started '
							# console.log(require('util').inspect(decoded_value, false, 4))
							# logger.info('rtmp: Game has not started')
							cmd.args.push({error:'OB-3'})
				# 		else
