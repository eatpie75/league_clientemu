...
class Decoder extends AMFContext
	...
	readValue: () ->
		...
		value = switch type
			...
			when TYPE_BYTEARRAY then @readByteArray()
				...
				# logger.warn("AMF3: Undefined type:#{type}")
				@readUndefined()
		return value
		...
		if (ref & 1)
			...
			if (ref & 2)
				...
				if (ref & 4)
					...
					if klass.name in ['com.riotgames.platform.systemstate.ClientSystemStatesNotification', 'DSA']
						return object
	...
	_getClassDefinition: (ref) ->
		...
		else if name is 'com.riotgames.platform.broadcast.BroadcastNotification'
		else if name is 'com.riotgames.platform.systemstate.ClientSystemStatesNotification'
		else if name is 'DSA'
