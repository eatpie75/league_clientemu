...
(function() {
  var AMF0Decoder, AMF0Encoder, RTMPClient, RTMPCommand, RTMPHeader, RTMPMessage, RTMPProtocol, assert, events, hexy, logger,
  ...
  RTMPProtocol = (function() {
    ...
    function RTMPProtocol(_at_stream) {
    ...
    RTMPProtocol.prototype.parseMessages = function(data) {
      var CHANNEL_MASK, ack, channel, count, e, hdr, hdrsize, hdrtype, header, msg, newData, offset, oldData, packetData, size, sizeData, timestamp, timestampData;
      CHANNEL_MASK = 0x3F;
      offset = 0;
      msg = null;
      while (offset < data.length) {
        ...
        if (hdrtype < RTMPHeader.TIME) {
          ...
          try {
            ...
            e = _error;
            logger.info('rtmp: 152:data.copy(sizeData, 1, offset, offset+3)', e);
            logger.info("rtmp: sizeData:" + sizeData + ", offset:" + offset);
          }
          offset += 3;
  ...
  RTMPClient = (function(_super) {
    ...
    function RTMPClient(_at_socket) {
    ...
    RTMPClient.prototype.handshake = function(cb) {
      ...
      handshake1[0] = 0x03;
      clientResponse = null;
      handleHandshake1 = (function(_this) {
        return function(data) {
      ...
      handleHandshake2 = (function(_this) {
        return function(data) {
    ...
    RTMPClient.prototype.parseData = function(data) {
      var CHANNEL_MASK, buf, channel, dataNeeded, e, extraData, hdrsize, hdrtype, header, newData, offset, sizeData, timestamp, timestampData, totalSize;
      extraData = null;
      if (this.partialMessage) {
        ...
        if (hdrtype < RTMPHeader.TIME) {
          ...
          try {
            ...
            e = _error;
            logger.info('rtmp: 361:data.copy(sizeData, 1, offset, offset+3)', e);
            logger.info("rtmp: sizeData:" + sizeData + ", offset:" + offset);
          }
          offset += 3;
          header.size = sizeData.readUInt32BE(0);
      ...
      while (offset < data.length) {
        ...
        if (packetData.length < header.size) {
          ...
          if (msg.type === RTMPMessage.ACK) {
            logger.info('rtmp: 554:msg.type==RTMPMessage.ACK');
            logger.info(hexy.hexy(data));
          }
          return msg;
        }
      }
    ...
    RTMPClient.prototype.send = function(cmd, cb) {
      ...
      closeSocket = (function(_this) {
        return function() {
          var key, timeout, _ref1, _results;
          logger.error('rtmp: Timeout reached, closing socket.');
          if (!_this.broken) {
            _this.broken = true;
            _this.socket.destroySoon();
            _this.ev.emit('timeout');
            process.exit(5);
          }
    ...
    RTMPClient.prototype.messageReceived = function(msg) {
      ...
      if ((msg.type === RTMPMessage.RPC || msg.type === RTMPMessage.RPC3) && msg.streamId === 0) {
        ...
        logger.warn('rtmp: Got ack');
        this.socket.destroySoon();
        this.ev.emit('throttled');
        process.exit(3);
        this.writeWinSize0 = this.writeWinSize;
        return this.writeWinSize = msg.data.readUInt32BE(0);
      } else if (msg.type === RTMPMessage.WIN_ACK_SIZE) {
        this.readWinSize = msg.data.readUInt32BE(0);
        return this.readWinSize0 = this.bytesRead;
      } else {
  ...
  RTMPCommand = (function() {
    function RTMPCommand(_at_type, _at_name, _at_id, _at_data, _at_args) {
      this.type = _at_type;
      this.name = _at_name;
      this.id = _at_id;
      this.data = _at_data;
    ...
    RTMPCommand.fromMessage = function(message) {
      var cmd, data, decoded_value, decoder, errors, tmp;
      ...
      if (typeof cmd.id === 'undefined' || typeof cmd.data === 'undefined') {
        process.exit(3);
      }
      cmd.args = [];
      if (cmd.name === '_error') {
        decoded_value = decoder.readValue();
        if (decoded_value.hasOwnProperty('object')) {
          if (decoded_value.object.hasOwnProperty('rootCause')) {
            tmp = decoded_value.object.rootCause.object;
            if (tmp.errorCode === 'LOGIN-0007') {
              process.exit(4);
            } else if (tmp.errorCode === 'LOGIN-0001') {
              process.send({
                'event': 'new_version',
                'data': tmp.substitutionArguments[1]
              });
            } else if (tmp.errorCode === 'PG-0014') {
              cmd.args.push({
                error: 'OB-1'
              });
            } else if (tmp.rootCauseClassname === 'com.riotgames.platform.game.GameObserverModeNotEnabledException') {
              if (tmp.message === 'Game is not observable ') {
                cmd.args.push({
                  error: 'OB-2'
                });
              } else if (tmp.message === 'Game has not started ') {
                cmd.args.push({
                  error: 'OB-3'
                });
              }
          ...
          logger.info('rtmp:', decoded_value);
          return cmd;
        }
      }
      errors = 0;
      while (decoder.offset < decoder.buffer.length) {
        tmp = decoder.readValue();
        if (typeof tmp === 'undefined') {
          errors += 1;
          if (errors > 75) {
            logger.error('rtmp: undefined storm');
            process.exit(3);
          } else if (errors === 10) {
            logger.warn('rtmp: 10 undefineds');
          }
        }
        cmd.args.push(tmp);
      }
  ...
  RTMPMessage = (function() {
    ...
    function RTMPMessage(_at_header, _at_data) {
