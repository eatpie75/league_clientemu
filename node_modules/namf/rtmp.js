...
(function() {
  ...
  RTMPCommand = (function() {
    ...
    RTMPCommand.fromMessage = function(message) {
      var cmd, data, decoder, errors, tmp;
      ...
      if (cmd.name === '_error') {
        ...
        if (tmp.hasOwnProperty('object')) {
          if (tmp.object.hasOwnProperty('rootCause')) {
            if (tmp.object.rootCause.object.errorCode === 'LOGIN-0007') {
              process.exit(4);
            } else if (tmp.object.rootCause.object.errorCode === 'PG-0014') {
              cmd.args.push({
                error: 'OB-1'
              });
            } else if (tmp.object.rootCause.object.rootCauseClassname === 'com.riotgames.platform.game.GameObserverModeNotEnabledException') {
              if (tmp.object.rootCause.object.message === 'Game is not observable ') {
                cmd.args.push({
                  error: 'OB-2'
                });
              } else if (tmp.object.rootCause.object.message === 'Game has not started ') {
                cmd.args.push({
                  error: 'OB-3'
                });
              } else {
                logger.info('rtmp:', tmp);
              }
            } else {
              logger.info('rtmp:', tmp);
              return cmd;
