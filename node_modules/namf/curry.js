// Generated by CoffeeScript 1.4.0
(function() {
  var AMF0Decoder, AMF0Encoder, enc, logAll, logger, today;

  logger = require('winston');

  today = function() {
    var day, weekdays;
    weekdays = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
    day = (new Date).getDay();
    return weekdays[day];
  };

  AMF0Encoder = (require('./amf0')).Encoder;

  AMF0Decoder = (require('./amf0')).Decoder;

  enc = new AMF0Encoder();

  if (today() === 'Wednesday') {
    enc.writeString('Yay its curry day!');
  } else {
    enc.writeString('Ah crap, its not curry day :(');
  }

  enc.writeString('The answer to life, the universe and everything');

  enc.writeNumber(42);

  enc.writeNull();

  enc.writeUndefined();

  enc.writeBoolean(true);

  enc.writeBoolean(false);

  enc.writeString('All done, yo.');

  logger.info('curry: The encoded binary data:', enc.getBuffer());

  logAll = function(buffer) {
    var dec, _results;
    dec = new AMF0Decoder(buffer);
    _results = [];
    while (dec.offset < buffer.length) {
      _results.push(logger.info('curry: ', dec.readValue()));
    }
    return _results;
  };

  logger.info("curry: Decode it:");

  logAll(enc.getBuffer());

}).call(this);
